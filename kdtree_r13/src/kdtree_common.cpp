...
//  mexPrintf("build_kdtree started!\n");
...
//  mexPrintf("created internal node!\n");
#ifdef DEBUG_BUILD_TREE
...
Node * rangeQuery(Node *v, double distance, double *pt, int D, int depth){
  ...
  if (v->left){
    if ( v->pt[v->orientation] >= (pt[v->orientation] - distance ) ){
      if (tmp)
	tmp->right=rangeQuery(v->left, distance, pt,D,depth+1);
      else
	current=rangeQuery(v->left, distance, pt, D,depth+1);
    }
  }
  ...
  if (v->right){
    if (   v->pt[v->orientation]  <= (pt[v->orientation] + distance ) ){
      if (tmp)
	tmp->right=rangeQuery(v->right, distance, pt, D,depth+1);
      else
	current=   rangeQuery(v->right, distance, pt, D,depth+1);
    }
  }    
...
void run_queries( Node *pVertex, double *model, int M, int D, 
  ...
  for (i=0; i < M; i++) {
    ...
    LL=rangeQuery(pVertex,min_distance,pt,D,0);
...
void run_range_search( Node *pVertex, double *model, int M, int D, 
  ...
  for (i=0; i < M; i++) {
    ...
    for (j=0; j < D ; j++)
      pt[j]=model[M*j+i];
    LL=rangeQuery(pVertex,distlim,pt,D,0);
